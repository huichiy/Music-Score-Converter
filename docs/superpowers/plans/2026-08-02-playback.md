...
describe('buildPlaybackEvents: linear measures → timed events (beats)', () => {
  ...
  // The measure must be full: event times are pure accumulation of written
  // durations (no padding to the bar), so a one-note measure would put the
  // second pass at beat 1, not 4.
  const rep: Measure[] = [
...
Run: `npx tsx scripts/test-roundtrip.ts`
Expected: PASS — results line shows **178 passed, 0 failed**.

- [ ] **Step 5: Verify tests + build**

Run: `npm run test && npm run build`
Expected: `Results: 178 passed, 0 failed`, `Results: 31 passed, 0 failed`, `✓ built in …`.
...
Expected: `✓ built in …`, then `Results: 178 passed` and `Results: 31 passed`. Also confirm the main bundle did not absorb Tone: the build output should list a separate lazy chunk, and `dist/assets/index-*.js` should stay near its previous size (~389 KB).
...
Run: `npm run build && npm run test 2>&1 | grep "Results:"`
Expected: `✓ built in …`, `Results: 178 passed`, `Results: 31 passed`.
...
```tsx
  const play = useCallback(async (fromBeat?: number) => {
    ...
    // resuming at the pause point — seekToNote routes the paused case here.
...
Also correct the alias guidance in the Local Dev section — find:

```
- Imports use the `@/` alias → `src/`; follow it in new files.
```

Replace with:

```
- Imports use the `@/` alias → `src/`; follow it in new files, with one caveat: **value** imports between files under `src/lib/` must be relative (`./parser`), because the tsx test runner doesn't resolve the alias for value imports. Type-only imports (`import type … from '@/types/score'`) are always fine.
```
...
**Assertion counts used in verify steps:** 130 → 141 (Task 1) → 150 (Task 2) → 178 (Task 3), then steady. If a count differs by a couple, trust the runner and update the docs figure in Task 7 to match rather than forcing the number.

**Corrections made during execution** (the plan was wrong; these are the rulings):
1. **Under-filled measures are NOT padded to the bar.** Event times are pure accumulation of written durations. The first draft's repeat fixture (one measure holding a single quarter note) expected the second pass at beat 4, which would require padding — and that contradicted the temp-time-signature test's `1 + 6 = 7`, which only holds without padding. The two were arithmetically mutually exclusive. Ruling: keep accumulation (padding would fill a dropped note with phantom silence, masking the very error playback exists to expose) and make the repeat fixture a genuinely full 4/4 measure so its assertion means what it says.
2. **Value imports inside `src/lib/` must be relative** (`./parser`), not `@/lib/parser`: the tsx test runner does not resolve Vite's `@/` alias for value imports. Type-only `@/types/score` imports are fine (erased at compile). Precedent: `src/lib/vision/index.ts`. CLAUDE.md's blanket "use the `@/` alias in new files" needs this caveat — Task 7 adds it.
3. Assertion count for Task 3 is 28 new, not 26 (→ 178, not 176).
